...
for i in range(len(cleanupAllocations)):# loops through each line in the input file
    ...
    test1 = any(x in group2Range for x in group1Range)# checks if any of the rooms the first group will be cleaning are already allocated to the second group
    test2 = any(x in group1Range for x in group2Range)# checks if any of the rooms the second group will be cleaning are already allocated to the first group
